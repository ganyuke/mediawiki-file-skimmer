...
from app.databags import AppDeps, PublishPayload, PublishStage, StatusState
...
class UploadQueue:
    ...
    async def _eat_queue(self, payloads: list[PublishPayload]):
        ...
        for payload in payloads:
            if self._panic_button.is_set():
                ...
                return
            await self._post(payload, edit_client)

        self._notifier(None, PublishStage.DONE, StatusState.INVALID)

    def publish_all(self, payloads: list[PublishPayload]) -> None:
        if (len(payloads) <= 0):
        ...
        def async_task():
            ...
            self._running_task = self._deps.event_loop.create_task(self._eat_queue(payloads))
        _ = self._deps.event_loop.call_soon_threadsafe(async_task)

    async def _post(self, payload: PublishPayload, edit_client: MediaWikiEditClient):
        ...
        if (payload.new_text is not None):
            ...
            result = await edit_client.edit_page(title=payload.original_title, new_text=payload.new_text, summary=payload.summary)
        ...
        if (payload.new_title is not None):
            ...
            result = await edit_client.move_page(title=payload.original_title, new_title=payload.new_title, reason=payload.reason)
